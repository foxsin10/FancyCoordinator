...
public extension CoordinatorBuilder {
  struct Sequenced<C0: CoordinatorRepresentable, C1: CoordinatorRepresentable>: CoordinatorRepresentable
  ...
  struct SequensedMany<Element: CoordinatorRepresentable>: CoordinatorRepresentable {
    @usableFromInline
  ...
  enum ConditionCoordinator<First: CoordinatorRepresentable, Second: CoordinatorRepresentable>: CoordinatorRepresentable
    ...
    public func coordinate(to route: First.Route, withContext context: First.Context) -> First.Scene? {
      switch self {
      case let .first(first): first.coordinate(to: route, withContext: context)
      case let .second(second): second.coordinate(to: route, withContext: context)
      }
    }
  }

  struct OptionalCoordinator<Wrapped: CoordinatorRepresentable>: CoordinatorRepresentable
